...
class ControllerMethodTest {
  ...
  @Test
  def primitiveParamCallJavaSyntax() {
    val meth = ControllerMethod("controllers.Application.index", List(("param1", "long")))
    Assert.assertEquals("Unexpected routeCallSyntax", "controllers.Application.index(param1: Long)", meth.toRouteCallSyntax)

    val meth1 = ControllerMethod("controllers.Application.index", List(("param1", "int")))
    Assert.assertEquals("Unexpected routeCallSyntax", "controllers.Application.index(param1: Int)", meth1.toRouteCallSyntax)
  }
